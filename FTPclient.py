...
dataPort = 2222
buffer = 1024
# communication control socket and data transfer socket
...
def Upload(file_path):
    ...
    fileName = os.path.basename(os.path.normpath(file_path)) #extract file name from path
...
def Download(fileName):
    controlSocket.sendall(str("Download").encode('utf-8')) #send download command
    controlSocket.recv(buffer)
    dataSocket.connect((IP, dataPort))
    print("You Are Connected To The Server")
    controlSocket.sendall(fileName.encode('utf-8'))
    fileSize = struct.unpack("i", controlSocket.recv(buffer))[0] #recieve file size from server
    file_path = file_path = os.path.join(".", "Download Folder", fileName) #create path
    file = open(file_path, "wb") #create local file in binary write mode
    comingSize = 0
    print("Downloading...")
    while comingSize < fileSize: #recieve data chuncks from the server and write them 
...
def Pwd():
    controlSocket.sendall(str("Pwd").encode('utf-8')) #send pwnd command
    controlSocket.recv(buffer)
    controlSocket.sendall(str(".").encode('utf-8'))
    path = controlSocket.recv(buffer).decode('utf-8') #get the current directory from the server
